#!/usr/bin/env bash
# examples/run_demos.sh — Test runner for topassuite (bash / Linux / macOS)
#
# Run from the REPO ROOT with the conda env active:
#   conda activate topassuite
#   cd /path/to/TopasSuite-Python
...
if [[ "$SOLO" == "todo" || "$SOLO" == "real" ]]; then
    ...
    python -m topassuite.cli.main info "$F1" "$F2" "$F3" "$F4" "$F5" "$F6"

    titulo "REAL 2 — Info: JSON output"
    python -m topassuite.cli.main info --json "$F1" "$F2" "$F3" "$F4" "$F5" "$F6"
    ...
    python -m topassuite.cli.main export-image \
        "$F1" "$F2" "$F3" "$F4" "$F5" "$F6" \
    ...
    python -m topassuite.cli.main navline \
        "$F1" "$F2" "$F3" "$F4" "$F5" "$F6" \
