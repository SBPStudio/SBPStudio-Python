...
``sbp_studio/gui/translations/`` (e.g. ``sbp_studio_es.ts``), regenerated from
...
class TsTranslator(QTranslator):
    ...
    def load_lang(self, lang: str) -> bool:
        """Load ``sbp_studio_<lang>.qm`` if present, else parse the ``.ts``."""
        qm = _TRANSLATIONS_DIR / f"sbp_studio_{lang}.qm"
        if qm.exists() and self.load(str(qm)):
            self._map.clear()  # native .qm handles lookups
            return True
        ts = _TRANSLATIONS_DIR / f"sbp_studio_{lang}.ts"
        if ts.exists():
