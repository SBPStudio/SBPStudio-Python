...
def _app_root() -> str:
    ...
    * Source run → the PROJECT ROOT, derived from this file's package location
      (``…/topassuite/gui/components/cli_console.py`` → parents[3]). This is the
      ...
      topassuite.gui``, an IDE, etc.) — unlike ``dirname(sys.argv[0])`` which
...
class CliConsole(QWidget):
    ...
    def __init__(self, window: QWidget) -> None:
        ...
        header = QHBoxLayout()
        self._title = QLabel("›_  TopasSuite CLI")     # brand, not translated
    ...
    def _cmd_version(self, _args: List[str]) -> None:
        import topassuite
        self.println(f"TopasSuite {getattr(topassuite, '__version__', '?')}")
    ...
    # ── Core CLI bridge (reuses topassuite.cli argparse + handlers) ────────────

    def _wire_core_commands(self) -> None:
        """Map the legacy headless CLI commands into the console. They share the
        EXACT argparse logic of ``python -m topassuite.cli.main`` — so the user
    ...
    def _intro(self) -> None:
        self.println("TopasSuite integrated CLI — type 'help'.")
...
_GUIDE_HTML = f"""
<h2>Guía de Comandos — Consola interna de TopasSuite</h2>

<p>La consola interna ejecuta los <b>mismos comandos</b> que la herramienta de
línea de comandos de TopasSuite, pero <b>directamente dentro de la aplicación</b>.
Escribe el comando y sus argumentos y pulsa <b>Intro</b>.</p>

<h3>Formato</h3>
<pre>&lt;comando&gt; &lt;archivo(s)&gt; [--opción valor] [--bandera]</pre>
<ul>
  <li><b>NO</b> hace falta el prefijo <code>python -m topassuite.cli.main</code>.
