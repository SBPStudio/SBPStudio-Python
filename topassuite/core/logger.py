...
Until ``configure_logging`` runs, the ``topassuite`` logger carries only a
...
def get_logger(name: Optional[str] = None) -> logging.Logger:
    """Return the shared ``topassuite`` logger, or a dotted child (e.g.
    ``get_logger("io_segy")`` → ``topassuite.io_segy``)."""
...
def configure_logging(level: int = logging.INFO,
    ...
    the ``topassuite`` logger and return it.
